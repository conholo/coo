...
VulkanSimpleRenderer::VulkanSimpleRenderer(VulkanRenderer *renderer)
    :m_Renderer(renderer)
{
...
void VulkanSimpleRenderer::CreateSimpleTexture()
{
    ...
    m_SimpleTexture = VulkanTexture2D::CreateFromMemory(textureSpec, textureData);
    ...
    m_SimpleTexture = VulkanTexture2D::CreateFromFile(spec, "../assets/textures/missing.png");
}
...
void VulkanSimpleRenderer::CreateSimpleResources()
{
    ...
    CreateOrInvalidateSimpleFramebuffers();
}
...
void VulkanSimpleRenderer::CreateSimplePipeline()
{
    ...
    auto builder =
            VulkanGraphicsPipelineBuilder("Simple Pipeline")
...
void VulkanSimpleRenderer::CreateOrInvalidateSimpleFramebuffers()
{
...
void VulkanSimpleRenderer::Render(FrameInfo& frameInfo)
{
    ...
    m_SimplePass->BeginPass(frameInfo.CommandBuffer, renderPassInfo, m_Renderer->VulkanSwapchain());
            ...
            {
                {
                    ...
                    .imageInfo = m_SimpleTexture->GetBaseViewDescriptorInfo()
                }
            }
           }
       });
    m_SimpleBaseMaterial->BindDescriptors(frameInfo.FrameIndex, frameInfo.CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS);
    vkCmdDraw(frameInfo.CommandBuffer, 3, 1, 0, 0);
    m_SimplePass->EndPass(frameInfo.CommandBuffer);
}
